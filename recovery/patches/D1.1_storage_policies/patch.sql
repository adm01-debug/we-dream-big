-- ═════════════════════════════════════════════════════════════════
-- PATCH D.1.1 — Storage Buckets + Policies (consolidado idempotente)
-- Origem: recovery/block09_storage.sql + recovery/block09b_storage_policies_full.sql
-- Transação atômica: tudo ou nada
-- ═════════════════════════════════════════════════════════════════

BEGIN;

-- Pré-checks (abort se algo crítico não existir)
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace
                 WHERE n.nspname='public' AND p.proname='is_supervisor_or_above') THEN
    RAISE EXCEPTION 'Function public.is_supervisor_or_above() não existe — patch abortado';
  END IF;
END $$;
