...
export function useGlobalSearch() {
  ...
  const performSemanticSearch = useCallback(async (searchQuery: string) => {
    ...
    try {
          ...
          if (intent.filters.priceRange) {
            if (intent.filters.priceRange === 'low')
              filteredProducts = filteredProducts.filter(
                (p) =>
                  ((p as ExternalProduct).sale_price || (p as ExternalProduct).base_price || 0) < 50,
              );
            else if (intent.filters.priceRange === 'high')
              filteredProducts = filteredProducts.filter(
                (p) =>
                  ((p as ExternalProduct).sale_price || (p as ExternalProduct).base_price || 0) > 200,
              );
          }
        ...
        try {
          ...
          if (searchTerm) {
            const companies = await searchCrm<Record<string, string>>(
              'companies', 'razao_social', searchTerm,
              { select: 'id, razao_social, nome_fantasia, ramo', limit: 5 },
            );
            companies.forEach((c) => {
              allResults.push({ id: c.id, title: c.nome_fantasia || c.razao_social, subtitle: c.ramo || 'Empresa', type: 'client', href: `/cliente/${c.id}` });
            });
          }
        } catch { /* silent */ }
      }
      ...
      if (intent.type === 'quote' || intent.type === 'mixed') {
        try {
          ...
          const quotes = await selectCrm<Record<string, unknown>>('quotes', {
            filters, orderBy: { column: 'created_at', ascending: false }, limit: 5,
          });
      ...
      const simpleQueries: Array<{ type: SearchResultType; table: string; select: string; filter?: string; titleField: string; subtitleField?: string; hrefPrefix: string }> = [
        { type: 'collection', table: 'collections', select: 'id, name, description, icon', titleField: 'name', subtitleField: 'description', hrefPrefix: '/colecoes/' },
        { type: 'conversation', table: 'expert_conversations', select: 'id, title, updated_at', titleField: 'title', hrefPrefix: '/expert?conversation=' },
        { type: 'category', table: 'category_icons', select: 'id, category_name, description, icon', titleField: 'category_name', subtitleField: 'description', hrefPrefix: '/?category=' },
      ];

      for (const q of simpleQueries) {
        if (wants(q.type)) {
          try {
            let builder = supabase.from(q.table).select(q.select);
            if (q.type === 'category') builder = (builder as any).eq('is_active', true).or(`category_name.ilike.%${term}%,description.ilike.%${term}%`);
            else if (q.type === 'collection') builder = (builder as any).ilike('name', `%${term}%`);
            else if (q.type === 'conversation') builder = (builder as any).ilike('title', `%${term}%`).order('updated_at', { ascending: false });
            const { data } = await builder.limit(5);
            (data || []).forEach((row: Record<string, unknown>) => {
              const id = row.id as string;
              allResults.push({
                id, type: q.type,
                title: (q.type === 'category' ? row.category_name : row[q.titleField]) as string || '',
                subtitle: q.subtitleField ? (row[q.subtitleField] as string) || '' : undefined,
                href: q.type === 'category' ? `${q.hrefPrefix}${encodeURIComponent(row.category_name as string)}` : `${q.hrefPrefix}${id}`,
                metadata: q.type === 'collection' || q.type === 'category' ? { icon: row.icon } : undefined,
              });
            });
          } catch { /* silent */ }
        }
      ...
      if (wants('kit')) {
        try {
          ...
          (data || []).forEach((k) => allResults.push({ id: k.id, title: k.name ?? '', subtitle: `${k.status} • ${k.kit_quantity}x • ${new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(k.total_price || 0)}`, type: 'kit', href: `/kits/${k.id}` }));
        } catch { /* silent */ }
      }
      if (wants('mockup')) {
        try {
          const { data } = await supabase.from('generated_mockups').select('id, product_name, client_name, technique_name, created_at').or(`product_name.ilike.%${term}%,client_name.ilike.%${term}%,technique_name.ilike.%${term}%`).order('created_at', { ascending: false }).limit(5);
          (data || []).forEach((m) => allResults.push({ id: m.id, title: m.product_name || 'Mockup', subtitle: `${m.client_name || 'Sem cliente'} • ${m.technique_name || '—'}`, type: 'mockup', href: `/mockups/${m.id}` }));
        } catch { /* silent */ }
      }
      if (wants('art_file')) {
        try {
          const { data } = await supabase.from('art_file_attachments').select('id, original_name, notes, file_url, quote_id').or(`original_name.ilike.%${term}%,notes.ilike.%${term}%`).order('created_at', { ascending: false }).limit(5);
          (data || []).forEach((a) => allResults.push({ id: a.id, title: a.original_name, subtitle: a.notes || 'Arquivo de arte', type: 'art_file', href: a.quote_id ? `/orcamentos/${a.quote_id}` : a.file_url, metadata: { file_url: a.file_url } }));
        } catch { /* silent */ }
      }
      if (wants('cart_template')) {
        try {
          const { data } = await supabase.from('cart_templates').select('id, name, description, items').or(`name.ilike.%${term}%,description.ilike.%${term}%`).limit(5);
          (data || []).forEach((t) => { const itemCount = Array.isArray(t.items) ? t.items.length : 0; allResults.push({ id: t.id, title: t.name, subtitle: `${itemCount} ${itemCount === 1 ? 'item' : 'itens'} • ${t.description || 'Template de carrinho'}`, type: 'cart_template', href: `/carrinho?template=${t.id}` }); });
        } catch { /* silent */ }
      }
      if (wants('magic_up')) {
        try {
          const { data } = await supabase.from('magic_up_generations').select('id, client_name, product_name, scene_title, scene_category, generated_image_url, created_at').or(`client_name.ilike.%${term}%,product_name.ilike.%${term}%,scene_title.ilike.%${term}%,scene_category.ilike.%${term}%`).order('created_at', { ascending: false }).limit(5);
          (data || []).forEach((m) => allResults.push({ id: m.id, title: m.scene_title || m.product_name || 'Magic Up', subtitle: `${m.client_name || 'Sem cliente'} • ${m.product_name || ''}${m.scene_category ? ' • ' + m.scene_category : ''}`, type: 'magic_up', href: '/magic-up', metadata: { image_url: m.generated_image_url } }));
        } catch { /* silent */ }
      }
      if (wants('component')) {
        try {
          const { data } = await supabase.from('product_components').select('id, component_code, component_name, product_id, is_personalizable').eq('is_active', true).or(`component_name.ilike.%${term}%,component_code.ilike.%${term}%`).limit(5);
          (data || []).forEach((c) => allResults.push({ id: c.id, title: c.component_name, subtitle: `Código: ${c.component_code}${c.is_personalizable ? ' • Personalizável' : ''}`, type: 'component', href: `/produto/${c.product_id}` }));
        } catch { /* silent */ }
      }
      if (wants('media')) {
        try {
          const { data } = await supabase.from('component_media').select('id, title, media_type, url, product_id').ilike('title', `%${term}%`).order('created_at', { ascending: false }).limit(5);
          (data || []).forEach((m) => allResults.push({ id: m.id, title: m.title || 'Mídia sem título', subtitle: `${m.media_type === 'video' ? '🎬 Vídeo' : '🖼️ Imagem'} • Componente`, type: 'media', href: m.product_id ? `/produto/${m.product_id}` : m.url, metadata: { url: m.url } }));
        } catch { /* silent */ }
      }
      ...
      const candidates = allResults.filter((r) => RERANK_TYPES.includes(r.type)).map((r) => ({ id: r.id, label: r.title, sublabel: r.subtitle ?? '' }));

      let finalResults = allResults;
      if (candidates.length > 1) {
        try {
          const { data: ranked } = await supabase.rpc('search_records_rerank', { _query: searchQuery, _candidates: candidates });
          if (ranked && Array.isArray(ranked) && ranked.length > 0) {
            ...
            const reranked = allResults.filter((r) => RERANK_TYPES.includes(r.type)).sort((a, b) => (scoreMap.get(b.id) ?? 0) - (scoreMap.get(a.id) ?? 0));
      ...
      void supabase.auth.getUser().then(({ data }) => {
        ...
        if (!userId) return;
        return supabase.from('search_analytics').insert({ user_id: userId, search_term: searchQuery.toLowerCase().trim().slice(0, 200), results_count: finalResults.length, search_context: JSON.stringify({ latency_ms: latencyMs, intent_type: intent.type }) }).then(() => undefined, () => undefined);
      }, () => undefined);
    } catch {
      setIsAIProcessing(false);
    } finally {
  ...
  useEffect(() => {
    if (debouncedQuery.startsWith('/')) {
      ...
      const matchedCommands = commands
        .filter((c) => c.command.toLowerCase().includes(lowerQuery) || c.keywords?.some((k) => k.toLowerCase().includes(lowerQuery.slice(1))))
        .map((c) => ({ id: c.id, title: c.label, subtitle: c.description, type: 'command' as const, href: `command:${c.id}`, metadata: { iconName: c.icon } }));
  ...
  const handleSelect = useCallback(
    (href: string, saveToHistory = true) => {
      ...
      if (saveToHistory && query.trim()) {
        addGlobalHistoryItem({ id: `history-${query.trim()}`, label: query.trim(), type: 'general' });
