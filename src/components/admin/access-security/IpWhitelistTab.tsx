      ...
      <CardContent className="space-y-4">
        <div className="flex gap-2 items-end">
          <div className="flex-1 space-y-1">
            ...
            <Input placeholder="192.168.1.100 ou 10.0.0.0/24" value={newIp} onChange={(e) => setNewIp(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAdd()} />
          </div>
          <div className="flex-1 space-y-1">
            <Label className="text-xs">Rótulo (opcional)</Label>
            <Input placeholder="Ex: Escritório SP" value={newIpLabel} onChange={(e) => setNewIpLabel(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleAdd()} />
          </div>
            ...
            <TableBody>
              {ips.map((ip) => (
                <TableRow key={ip.id}>
                  ...
                  <TableCell><Switch checked={ip.is_active} onCheckedChange={(checked) => onToggle(ip.id, checked)} /></TableCell>
                  <TableCell className="text-muted-foreground text-xs">{format(new Date(ip.created_at), "dd/MM/yyyy HH:mm", { locale: ptBR })}</TableCell>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>O IP <span className="font-mono font-bold">{ip.ip_address}</span> será removido da whitelist.</AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancelar</AlertDialogCancel>
                          <AlertDialogAction onClick={() => onRemove(ip.id)} className="bg-destructive text-destructive-foreground">Remover</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                  </TableCell>
