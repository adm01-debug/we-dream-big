...
function getMinQuantity(product: Product): number {
  return product.min_quantity ?? (product as Product & { minQuantity?: number }).minQuantity ?? 0;
}

function getStockStatus(product: Product): string | undefined {
  return (
    product.stock_status ?? (product as Product & { stockStatus?: string }).stockStatus ?? undefined
  );
}

function isKitProduct(product: Product): boolean {
  return Boolean(product.is_kit ?? (product as Product & { isKit?: boolean }).isKit);
}
...
export function CompareTableView({
  ...
  const eq = {
    ...
    publico: allEqual(
      products.map((p) => tagArray(p.tags, 'publicoAlvo').slice().sort().join('|')),
    ),
    datas: allEqual(
      products.map((p) => tagArray(p.tags, 'datasComemorativas').slice().sort().join('|')),
    ),
          ...
          <Table>
            ...
            <TableBody>
              ...
              <HighlightedNumberRow
                ...
                products={products}
                valueFn={(p) => Number(p.price ?? 0) * Number(getMinQuantity(p) || 1)}
                renderFn={(v) => formatCurrency(v)}
                mode="lower-is-better"
                subtitle="TCO"
              />
              ...
              <HighlightedNumberRow
                label="Lead time"
                products={products}
                valueFn={(p) => leadTimeProxy(getStockStatus(p))}
                renderFn={(v) =>
              ...
              <SimpleRow
                ...
                products={products}
                render={(p) => {
                  const s = getStockStatusLabel(getStockStatus(p) ?? '');
