...
import { type QuoteTemplate, type QuoteTemplateItem, useQuoteTemplates } from '@/hooks/quotes';
...
export function AdminTemplatesManager({ onEditTemplate }: AdminTemplatesManagerProps) {
  ...
  const calculateTotal = (template: QuoteTemplate) => {
    const itemsTotal = template.items.reduce((sum: number, item: QuoteTemplateItem) => {
      const personalizationCost =
        item.personalizations?.reduce(
          (pSum: number, p: { unitCost?: number; setupCost?: number }) =>
            pSum + (p.unitCost || 0) * item.quantity + (p.setupCost || 0),
          0,
        ) || 0;
