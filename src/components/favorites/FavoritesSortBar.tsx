...
export function FavoritesSortBar({
  value, onChange, onlyPriceDrops = false, onTogglePriceDrops, priceDropCount,
}: Props) {
  const showDropChip = onTogglePriceDrops && (priceDropCount === null || priceDropCount === undefined || priceDropCount > 0);
        ...
        >
          ...
          {typeof priceDropCount === "number" && priceDropCount > 0 && (
            <span className={cn(
              ...
              onlyPriceDrops ? "bg-success-foreground/20 text-success-foreground" : "bg-success/20 text-success"
            )}>
              {priceDropCount}
            </span>
          )}
