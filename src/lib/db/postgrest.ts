 ...
 *
 * FIX 2026-06-01 (countMode): Implemented countMode so totalEstimate works correctly.
 * Previously countMode was declared in InvokeOptions but never passed to PostgREST.
 */
...
const COLUMN_MAP: Record<string, Record<string, string>> = {
  tabela_preco_gravacao_oficial: {
    ativo: 'ativo', codigo_tabela: 'codigo_tabela', codigo_curto: 'codigo_curto',
    grupo_tecnica: 'grupo_tecnica', max_cores: 'max_cores', custo_setup: 'custo_setup',
    custo_manuseio: 'custo_manuseio', cobra_por_cor: 'cobra_por_cor',
    usa_faixa_dimensional: 'usa_faixa_dimensional', nome: 'nome', id: 'id',
    is_active: 'ativo', active: 'ativo', table_code: 'codigo_tabela',
    table_code_option: 'codigo_curto', table_fullcode: 'codigo_tabela',
    customization_type_name: 'grupo_tecnica', technique_name: 'grupo_tecnica',
    name: 'nome', max_colors: 'max_cores', setup_price: 'custo_setup',
    handling_price: 'custo_manuseio', price_by_color: 'cobra_por_cor',
    price_by_area: 'usa_faixa_dimensional', technique_id: 'id',
    max_area_width_cm: 'id', max_area_height_cm: 'id', max_area_width: 'id', max_area_height: 'id',
  },
  tecnicas_gravacao: {
    ativo: 'ativo', codigo: 'codigo', nome: 'nome', slug: 'slug',
    ordem_exibicao: 'ordem_exibicao', id: 'codigo', is_active: 'ativo', active: 'ativo',
    name: 'nome', code: 'codigo', display_order: 'ordem_exibicao',
  },
};
...
function remapFilters(resolvedTable: string, filters: Record<string, unknown>): Record<string, unknown> {
...
function mapRows<T>(resolvedTable: string, rows: T[]): T[] {
  if (resolvedTable === 'tecnicas_gravacao') {
    return rows.map((r) => {
      ...
      return { ...row, id: row.codigo, code: row.codigo, name: row.nome, is_active: row.ativo, display_order: row.ordem_exibicao } as T;
    });
  }
  return rows;
}
...
export async function dbInvoke<T>(options: InvokeOptions): Promise<InvokeResult<T>> {
  ...
  const remappedOrderCol = options.orderBy ? remapColumnName(table, options.orderBy.column) : undefined;

  // FIX-CATALOG-07 (2026-06-01): implement countMode so totalEstimate is correct.
  // Previously countMode was declared in InvokeOptions but never passed to PostgREST.
  // Supabase JS: select(cols, { count: 'exact'|'planned'|'estimated' }) requests Prefer: count=X header.
  const countOpt = options.countMode && options.countMode !== 'none' ? options.countMode : undefined;
  let query = countOpt
  ...
  if (remappedFilters) {
    for (const [key, value] of Object.entries(remappedFilters)) {
      if (Array.isArray(value)) { query = query.in(key, value); }
        ...
        else logger.warn(`[postgrest] operador desconhecido '${op}' para coluna '${key}' -- filtro ignorado`);
      } else { query = query.eq(key, value); }
    }
  }

  if (options.orderBy && remappedOrderCol) query = query.order(remappedOrderCol, { ascending: options.orderBy.ascending ?? true });
  if (typeof options.limit === 'number') { const from = options.offset || 0; query = query.range(from, from + options.limit - 1); }
  ...
  // FIX-CATALOG-07: return DB count when available (from countMode header), else records.length.
