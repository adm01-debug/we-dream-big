...
export const quoteService = {
  async fetchQuotes(userId: string, scope: string) {
    let query = supabase
      // rls-allow: scope='self' aplica .eq('seller_id', userId) abaixo; scope='all' é restrito a admins via RLS server-side
      .from('quotes')
      .select('*')
  ...
  async fetchQuote(quoteId: string): Promise<Quote | null> {
    const { data: quoteData, error: qErr } = await supabase
      // rls-allow: filtro por id único + RLS por seller_id no banco
      .from('quotes')
      .select('*')
      .eq('id', quoteId)
      .single();
  ...
  async createQuote(
    ...
    const { data: inserted, error: insErr } = await supabase
      // rls-allow: insertPayload já carrega seller_id derivado de userId (buildInsertPayload)
      .from('quotes')
  ...
  async updateQuote(quoteId: string, quote: Partial<Quote>, items: QuoteItem[]): Promise<Quote> {
    ...
    const { data: updated, error: updErr } = await supabase
      // rls-allow: filtro por id único + RLS por seller_id no banco bloqueia update cross-seller
      .from('quotes')
  ...
  async deleteQuote(quoteId: string) {
    // rls-allow: filtro por id único + RLS por seller_id no banco bloqueia delete cross-seller
