...
const { mockProducts } = vi.hoisted(() => ({
  mockProducts: [
    { id: 'p1', name: 'Cheap Product', price: 10, stock: 100, images: [], colors: [], category_id: '1', variations: [] },
    { id: 'p2', name: 'Expensive Product', price: 1000, stock: 50, images: [], colors: [], category_id: '1', variations: [] },
    { id: 'p3', name: 'Medium Product', price: 500, stock: 200, images: [], colors: [], category_id: '1', variations: [] },
  ]
}));
  ...
  CatalogContent: vi.fn(({ paginatedProducts }) => (
    <div data-testid="catalog-content">
      {paginatedProducts.map((p: any) => (
        <div key={p.id} data-testid="product-item">{p.name}</div>
      ))}
    </div>
  ))
}));
...
// Mock hooks
vi.mock('@/hooks/products/useCatalogRealStats', () => ({ useCatalogRealStats: vi.fn(() => ({ data: null })) }));
vi.mock('@/hooks/products/useColorEnrichment', () => ({ useColorEnrichment: vi.fn(() => ({ data: new Map() })) }));
vi.mock('@/contexts/AuthContext', () => ({ useAuth: vi.fn(() => ({ user: { id: 'test-user' } })), AuthProvider: ({ children }: any) => <>{children}</> }));
vi.mock('@/contexts/SellerCartContext', () => ({ useSellerCartContext: vi.fn(() => ({ items: [] })), SellerCartProvider: ({ children }: any) => <>{children}</> }));
vi.mock('@/contexts/CollectionsContext', () => ({ useCollectionsContext: vi.fn(() => ({ collections: [], defaultColors: [], defaultIcons: [], isProductInCollection: () => false })), CollectionsProvider: ({ children }: any) => <>{children}</> }));
vi.mock('@/contexts/ProductsContext', () => {
  const ReactMock = require('react');
  const mockValue = { registerProducts: vi.fn(), products: mockProducts, getProductsByIds: vi.fn(() => []) };
  const Context = ReactMock.createContext(mockValue);
  return {
    useProductsContext: vi.fn(() => mockValue),
    useProductsContextSafe: vi.fn(() => mockValue),
    ProductsProvider: ({ children }: any) => <Context.Provider value={mockValue}>{children}</Context.Provider>,
    ProductsContext: Context,
  };
});
vi.mock('@/hooks/favorites', () => ({ useFavoriteQuickAdd: vi.fn(() => ({ handleQuickAdd: vi.fn() })) }));
