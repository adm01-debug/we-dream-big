...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'inbound_webhook_endpoints'
      AND column_name = 'secret_key'
  ) THEN
    ALTER TABLE public.inbound_webhook_endpoints ALTER COLUMN secret_key DROP NOT NULL;
  ELSE
    RAISE NOTICE '[inbound_webhook_endpoints] Skipped secret_key nullability: column is not present';
  END IF;
END $$;
...
INSERT INTO public.inbound_webhook_endpoints (id,slug,name,source_system,hmac_secret_ref,allowed_events,active,last_received_at,total_received,total_invalid,created_by,created_at,updated_at,metadata)
VALUES
