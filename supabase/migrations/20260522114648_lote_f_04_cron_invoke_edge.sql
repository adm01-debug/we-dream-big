...
BEGIN
  ...
  SELECT extensions.net_http_post(url:=v_url,headers:=jsonb_build_object('Content-Type','application/json','Authorization','Bearer '||v_anon),body:=p_body,timeout_milliseconds:=p_timeout_ms) INTO v_req;
  RETURN v_req;
END; $$;
