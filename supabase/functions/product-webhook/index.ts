...
const DEFAULT_BATCH_SIZE = Number(Deno.env.get("PRODUCT_WEBHOOK_BATCH_SIZE") ?? "200");
const MAX_BATCH_SIZE = 500;

const BATCH_SIZE = Number.isFinite(DEFAULT_BATCH_SIZE)
  ? Math.min(Math.max(Math.trunc(DEFAULT_BATCH_SIZE), 100), MAX_BATCH_SIZE)
  : 200;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (data.action) {
      case "upsert": {
        if (!singleProduct) throw new Error("Product data is required for upsert action");
      ...
      case "delete": {
        ...
        const { error: deleteError, count } = await supabase.from("products").delete().in("external_id", externalIds);
...
async function upsertProducts(supabase: any, products: ProductPayload[], chunkSize: number): Promise<UpsertOutcome> {
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    try {
      const externalIds = Array.from(new Set(chunk.map((item) => item.external_id).filter(Boolean))) as string[];
      const skus = Array.from(new Set(chunk.map((item) => item.sku).filter(Boolean)));

      let existingRows: Array<{ id: string; external_id: string | null; sku: string }> = [];
      ...
      const existingByExternalId = new Map(existingRows.filter((row) => row.external_id).map((row) => [row.external_id as string, row]));
      ...
      for (const item of chunk) {
        ...
        if (item.external_id) withExternalId.push(item);
        else withoutExternalId.push(item);
      }

      if (withExternalId.length > 0) {
        const { error: upsertByExternalError } = await supabase
          .from("products")
          .upsert(withExternalId, { onConflict: "external_id", ignoreDuplicates: false });
        chunkRoundtrips += 1;
      ...
      if (withoutExternalId.length > 0) {
        const { error: upsertBySkuError } = await supabase
          .from("products")
          .upsert(withoutExternalId, { onConflict: "sku", ignoreDuplicates: false });
        chunkRoundtrips += 1;
